...
跨平台全局热键 daemon，通过 AnkiConnect 控制 Anki 复习。

Linux:   通过 evdev 直接读取键盘设备（需加入 input 用户组）
Windows: 通过 keyboard 库注册全局热键（无需特殊权限）

依赖：
  Linux:   pip install evdev requests
  Windows: pip install keyboard requests
"""
import argparse
import asyncio
import atexit
...
class Config:
    def __init__(self):
        ...
        self.raw_hotkeys: "dict[str, str]" = {}
        self.hotkeys: dict = {}   # Linux only: {action: (mods, trigger)}
    ...
    def reload(self) -> bool:
        ...
        if sys.platform != "win32":
            self.hotkeys = {
                ...
                for action, hk_str in self.raw_hotkeys.items()
            }
        return True
...
if sys.platform != "win32":
    ...
    async def main_linux():
        ...
        for action in ACTIONS:
            print(f"  {action:12s}: {config.raw_hotkeys.get(action, '未设置')}", flush=True)
...
# ══════════════════════════════════════════════════════════════════
# Windows 后端（keyboard 库）
# ══════════════════════════════════════════════════════════════════
async def main_windows():
    try:
        import keyboard as kb
    except ImportError:
        print("[错误] 缺少 keyboard 库，请运行：pip install keyboard", flush=True)
        sys.exit(1)
    ...
    def register_all(cfg: Config):
        ...
        for handle in registered:
            try:
                ...
                pass
        registered = []
        mode = "拦截模式" if cfg.suppress else "普通模式"
        print(f"\n[模式] {mode}", flush=True)
        for action, hk_str in cfg.raw_hotkeys.items():
            try:
                handle = kb.add_hotkey(hk_str, ACTIONS[action], suppress=cfg.suppress)
                registered.append(handle)
                print(f"[热键] 已注册: {hk_str} → {action}", flush=True)
            except Exception as e:
                print(f"[警告] 注册热键失败 '{hk_str}': {e}", flush=True)
    ...
    try:
        while True:
            ...
            if config.reload():
                print("[配置] 检测到变更，重新注册热键", flush=True)
                register_all(config)
    except asyncio.CancelledError:
        pass
    finally:
